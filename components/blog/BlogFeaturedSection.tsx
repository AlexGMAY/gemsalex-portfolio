...
const BlogFeaturedSection = ({ allPosts }: BlogFeaturedSectionProps) => {
  const [activeCategory, setActiveCategory] = useState<string>("All");
  const [posts, setPosts] = useState<BlogPost[]>(allPosts || []);
      ...
      const counts = await Promise.all(
        posts.map(async (post) => {
          try {
            ...
            console.error(`Error loading shares for post ${post.id}:`, error);
  ...
  // Get filtered posts with proper fallbacks
  const filteredPosts =
    activeCategory === "All"
      ? [...posts]
      : posts.filter((post) => post.tags?.includes?.(activeCategory) ?? false);

  // Get featured post with proper fallbacks
  const featuredPost =
    filteredPosts.length > 0
      ? [...filteredPosts].sort((a, b) => (b.views || 0) - (a.views || 0))[0]
      : null;

  // Get trending posts with proper fallbacks
  const trendingPosts =
    filteredPosts.length > 0
      ? [...filteredPosts]
          .filter((post) => post.id !== featuredPost?.id)
          .sort((a, b) => {
            const aScore =
              (a.views || 0) * 0.7 + (shareCounts[a.id] || 0) * 0.3;
            const bScore =
              (b.views || 0) * 0.7 + (shareCounts[b.id] || 0) * 0.3;
            return bScore - aScore;
          })
          .slice(0, 3)
      : [];
  ...
  if (isLoading) {
    return (
      <section className="pt-36 pb-20 bg-gradient-to-b from-black-100 to-black-100 relative">
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center py-20">Loading featured posts...</div>
        </div>
      </section>
    );
  }
          ...
          >
            ...
            <div className="relative group h-full">
              ...
              <div className="relative h-full flex flex-col md:flex-row gap-6 p-6 bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700 group-hover:border-lime-400/30 transition-all">
                <div className="md:w-1/2 overflow-hidden rounded-xl">
                  <motion.img
                    src={featuredPost.image || "/default-post-image.jpg"}
                    alt={featuredPost.title}
                    className="w-full h-64 md:h-full object-cover rounded-xl group-hover:scale-105 transition-transform"
                  />
                </div>
